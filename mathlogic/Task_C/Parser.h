...
bool equal(tree_string* t, tree_string* s){
	if (t->op != s->op)
		return false;
	if (t->op == Variable){
		if (t->str == s->str)
	...
	if (t->left != NULL && s->left != NULL){
		l = equal(t->left, s->left);
	}
	if (l == 0 || l == -1){
		return false;
	}
	if (s->right != NULL && t->right != NULL){
		r = equal(t->right, s->right);
	}
	if (r == 0){
		return false;
	}
...
void clean(tree_string* tree){
	if (tree->left) clean(tree->left);
	if (tree->right) clean(tree->right);
	delete tree;
}
...
bool is_axiom(tree_string* t){
	//#1 A->B->A
	if (t->op == Implication){
		if (t->right->op == Implication){
			if (equal(t->left, t->right->right)){
				//cout << "1" << endl;
				return true;
			}	
		}
	...
	//#2 (A->B)->(A->B->Y)->(A->Y)
	if (t->op == Implication && t->left->op == Implication && t->right->op == Implication){
		if (t->right->right->op == Implication && t->right->left->op == Implication){
			if (t->right->left->right->op == Implication){
				if (equal(t->left->left, t->right->left->left) && equal(t->left->left, t->right->right->left)){
					if (equal(t->left->right, t->right->left->right->left)){
						if (equal(t->right->right->right, t->right->left->right->right)){
							//cout << "2" << endl;
							return true;
						}
					}
	...
	//#3 A->B->A&B
	if (t->op == Implication){
		if (t->right->op == Implication && t->right->right->op == Conjunction){
			if (equal(t->left, t->right->right->left) && equal(t->right->left, t->right->right->right)){
				//cout << "3" << endl;
				return true;
			}
		}
	...
	//#4 A&B->A, #5 A&B->B
	if (t->op == Implication && t->left->op == Conjunction){
		if (equal(t->right, t->left->left) || equal(t->right, t->left->right)){
			//cout << "4/5" << endl;
			return true;
		}		
	}
	//#6 A->A|B, #7 B->A|B
	if (t->op == Implication && t->right->op == Disjunction){
		if (equal(t->left, t->right->left) || equal(t->left, t->right->right)){
			//cout << "6/7" << endl;
			return true;
		}	
	}
	//#8 (A->Y)->(B->Y)->(A|B->Y)
	if (t->op == Implication && t->left->op == Implication && t->right->op == Implication){
		if (t->right->left->op == Implication && t->right->right->op == Implication && t->right->right->left->op == Disjunction){
			if (equal(t->left->left, t->right->right->left->left)){
				if (equal(t->right->left->left, t->right->right->left->right)){
					if (equal(t->left->right, t->right->left->right) && equal(t->left->right, t->right->right->right)){
						//cout << "8" << endl;
						return true;
					}
				}
	...
	//#9 (A->B)->(A->!B)->!A
	if (t->op == Implication && t->left->op == Implication && t->right->op == Implication){
		if (t->right->right->op == Negation && t->right->left->op == Implication && t->right->left->right->op == Negation){
			if (equal(t->left->left, t->right->right->left) && equal(t->left->left, t->right->left->left)){
				if (equal(t->left->right, t->right->left->right->left)){
					//cout << "9" << endl;
					return true;
				}
	...
	//#10 !!A->A
	if (t->op == Implication && t->left->op == Negation && t->left->left->op == Negation){
		if (equal(t->right, t->left->left->left)){
			//cout << "10" << endl;
			return true;
		}	
	}
